...
def Positions(doc):
    ...
    #os.chdir("/media/aail/OS/MSCS_NEU/Semester1/InformationRetreival_NadaNaji/Project/ImplementationPart1/SP")
    file = open(doc+'.txt', 'r')
    global query
    queryNew = query.lower()
    queryNew = nltk.word_tokenize(queryNew)
    docTerms = file.read().split()
...
def ranking(q):
    ...
    m = 0
    #os.chdir(os.path.join(sys.path[0], "Results_WithTermPositions_WithoutStopping"))
    ...
    for i in keys:
        ...
        if m > 99:
            ...
            m = m + 1
            rank = str(m)
            print(str(q_id) + " Q0" " " + i + " " + rank + " " + str(docidbm[i]) + " WithTermPositions_WithoutStopping" + "\n")
            #file.write(str(q_id) +" Q0" " " + i + " " + rank + " " + str(docidbm[i])+ " WithTermPositions_WithoutStopping"  + "\n")
