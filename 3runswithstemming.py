...
def dlandavdl():
    ...
    v = os.path.join(sys.path[0], "Stem-SP")
    os.chdir(v)
    y = os.listdir(v)
    for i in y:
        m = m + 1
...
def docwithtermfreq():
    x = os.path.join(sys.path[0], "projonegramtermstem.txt")
...
def bmranking(query, q_id):
    ...
    for i in range(1, len(docnames)):
        ...
        for q in query:
            ...
            bm = bm + score_BM25(termdocfreq[q], int(t), query[q], r, 3204, dl[nam], avdl)

        docidbm[nam] = docidbm[nam] + bm

    keys = sorted(docidbm, key=docidbm.get, reverse=True)
    m = 0
    #os.chdir(os.path.join(sys.path[0], "bm25stem_result"))
    ...
    for i in keys:
        ...
        if m > 99:
            ...
            #file.write(str(q_id) + " Q0" " " + i + " " + rank + " " + str(docidbm[i]) + " BM25-stem" + "\n")
...
def tfidfranking(query, q_id):
    ...
    #os.chdir("C:\\Users\\Rahul\\PycharmProjects\\IRpr\\tfidfstem_result")
    #os.chdir(os.path.join(sys.path[0], "tfidfstem_result"))
    #file = open(str(q_id) + ".txt", 'w', encoding='utf-8')
    for i in keys:
        ...
        if m > 99:
            ...
            #file.write(str(q_id) + " Q0 "  + i + " " + rank + "  " + str(docidtfidf[i]) + " TF-IDF-stem " + "\n")
    #file.close()
...
def smoothedquery(query, q_id):
    ...
    m = 0
    # os.chdir(os.path.join(sys.path[0], "smqstem_result"))
    # file = open(str(q_id) + ".txt", 'w', encoding='utf-8')
    for i in keys:
        ...
        if m > 99:
            ...
            m = m + 1
            rank = str(m)
            print(str(q_id) + " Q0  " + i + " " + rank + "  " + str(docidsmq[i]) + " smoothedT " + "\n")
            #file.write(str(q_id) + " Q0 " + i + " " + rank + "  " + str(docidsmq[i]) + " SMQ-stem " + "\n")
    #file.close()
...
def main():
    ...
    docwithtermfreq()
    print("Select")
    print("1 for BM25 for stem")
    print("2 for TFIDF for stem")
    print("3 for Smoothed_query for stem")
    print("q to quit")
    sel = input()

    if (sel == "1"):
        for qid in stemqid_query:
            query = stemqid_query[qid]
            q_id = qid
            query = query.rstrip()
