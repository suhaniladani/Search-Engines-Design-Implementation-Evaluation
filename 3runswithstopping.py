...
def bmranking(query, q_id):
    ...
    m = 0
    #os.chdir(os.path.join(sys.path[0], "bm25withstop_result"))
    ...
    for i in keys:
        ...
        if m > 99:
            ...
            m = m + 1
            rank = str(m)
            print(str(q_id) + " Q0" " " + i + " " + rank + " " + str(docidbm[i]) + " BM25stop" + "\n")
            #file.write(str(q_id) + " Q0" " " + i + " " + rank + " " + str(docidbm[i]) + " BM25Stop" + "\n")
...
def tfidfranking(query, q_id):
    ...
    m = 0
    #os.chdir(os.path.join(sys.path[0], "tfidfwithstop_result"))
    #file = open(str(q_id) + ".txt", 'w', encoding='utf-8')
    for i in keys:
        ...
        if m > 99:
            ...
            print(str(q_id) + " Q0" " " + i + " " + rank + "  " + str(docidtfidf[i]) + " TF-IDFstop " + "\n")
            #file.write(str(q_id) + " Q0 "  + i + " " + rank + "  " + str(docidtfidf[i]) + " TF-IDFstop " + "\n")
    #file.close()
...
def smoothedquery(query, q_id):
    ...
    #os.chdir(os.path.join(sys.path[0], "smqwithstop_result"))
    #file = open(str(q_id) + ".txt", 'w', encoding='utf-8')
    for i in keys:
        ...
        if m > 99:
            ...
            m = m + 1
            rank = str(m)
            print(str(q_id) + " Q0  " + i + " " + rank + "  " + str(docidsmq[i]) + " SMQstop " + "\n")
...
def query_ref(q):
    ...
    c = []
    for i in q:
        if i in termdocidtf and i not in stopwordlist:
            c.append(i)
    print(c)
